...
class Light():
    ...
    def _gen_code(self):
        ...
        payload = 'json.dumps({{"{brightness_o_property_name}": bool(data[0])}})'.format(
            brightness_o_property_name=brightness_o_property_name) if device_type == 'native' else 'bool(data[0])'
        brightness_o_request_code = "requests.put('{webthing_url}/properties/{brightness_o_property_name}', json={json_payload}, data={data_payload}, headers={headers})".format(
            webthing_url=self.webthing_url,
...
""".format(
            ...
            brightness_o_request_code=brightness_o_request_code)
        print(self.code)
